...
actor MatchRepository: MatchRepositoryProtocol {
    ...
    func fetchMatches() async throws -> [Match] {
        ...
        // Record cache miss
        await statistics.recordCacheMiss()
        ...
        await statistics.updateCacheSize(matchesCount: matches.count, oddsCount: cachedOdds.count)
    ...
    private func checkCacheValidityForMatches() -> Bool {
        if let timestamp = cacheTimestamp {
            ...
            if timeSinceCache > maxCacheAge || isEmpty {
                return true
            }
            
            // Use quick refresh logic for recent data
            let isStale = timeSinceCache > quickRefreshInterval
            
            // If data is stale but not too old, trigger background refresh
            if isStale && timeSinceCache < expirationInterval {
            ...
            // Cache is fresh
            let isValid = timeSinceCache < expirationInterval && !isEmpty
            return !isValid
        } else {
            return true
        }
    ...
    func fetchOdds() async throws -> [Odds] {
        ...
        // Update cache statistics
        await statistics.updateCacheSize(matchesCount: cachedMatches.count, oddsCount: odds.count)
    ...
    private func checkCacheValidityForOdds() -> Bool {
        if let timestamp = cacheTimestamp {
            let timeSinceCache = Date().timeIntervalSince(timestamp)
            let isValid = timeSinceCache < expirationInterval && !cachedOdds.isEmpty
            return !isValid
        } else {
            return true
        }
    ...
    private func warmupCache() async {
        ...
        do {
            ...
            // Update statistics
            await statistics.updateCacheSize(matchesCount: matches.count, oddsCount: odds.count)
    ...
    private func performBackgroundUpdate() async {
        ...
        if await timeSinceLastUpdate >= Constants.Cache.backgroundUpdateInterval {
            await backgroundRefresh()
        }
    }
